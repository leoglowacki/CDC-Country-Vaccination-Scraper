...
```{r}
rec_levels = c("All travelers", "Most travelers", "Some travelers")
...
make_country_df_tidy = function(df, rec_levels, country_name) {
  ...
  country %>% 
    #select(-c("Find.Out.Why", "Protect.Yourself")) %>% 
    select(-c(`Clinical Guidance for Healthcare providers`)) %>% 
    # mutate(X = remove_rest_of_colname(X)) %>% 
    # rowwise() %>% 
    # mutate(Rec_Level = make_rec_level(X, rec_levels)) %>% 
    # ungroup() %>% 
    # fill(Rec_Level) %>% 
    # filter(!(X %in% rec_levels)) %>% 
...
for (url in url_list) {
  ...
  if (!is.na(country_node %>% html_attr("class") == "disease")) {
    ...
    # drop any columns in new_df not in big_df
    #new_df = new_df %>% select(-setdiff(names(new_df), names(big_df)))
    # add our df for this one stock to the collective df
    big_df = big_df %>% add_row(tidy_country)
  } else {
    big_df = big_df %>% add_row(Country = country_name, Vaccine = NA)
  }
...
```{r}
big_df = big_df %>% distinct()
```


```{r}
write_csv(big_df, "full_country_vaccine_list.csv")
```
...
```{r}
big_df %>% drop_na() %>% filter(str_detect(Vaccine, "Malaria"))
```
